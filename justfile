...
build-release:
    ...
    cmake -DCMAKE_BUILD_TYPE=Release \
        ...
        -S . -B cmake-release
    cmake --build cmake-release --parallel
...
test-som: build-release
    cmake-release/SOM++ -cp Smalltalk TestSuite/TestHarness.som
...
hello: build-release
    cmake-release/SOM++ -cp Smalltalk Examples/Hello.som
...
awfy: build-release
    ...
    for b in Ball Bounce List Mandelbrot Permute Queens Sieve Storage Towers; do
        cmake-release/SOM++ -cp Smalltalk:Examples/AreWeFastYet Examples/AreWeFastYet/Harness.som $b 200 10 2>/dev/null | grep average
    done
...
clean:
    rm -rf cmake-release cmake-debug cmake-yk
